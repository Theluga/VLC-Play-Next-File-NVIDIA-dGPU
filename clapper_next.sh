...
# kill other instances of clapper
killall clapper; sleep 0.1

# launch empty clapper if no argument provided
if [ -z "$1" ]; then
	clapper; exit
fi
...
# if the filename does not have one of the extension above, launch clapper with provided filename
if [ -z "$pos" ]; then
	clapper -- "${filename}"
	exit
fi
...
n=$(echo "$list" | wc -l)
echo "$list" | tail -n$(($n-$pos+1)) >  /tmp/clapper.m3u
echo "$list" | head -n$(($pos-1))    >> /tmp/clapper.m3u

# launch playlist
IFS=$'\n'; read -d '' -r -a files < /tmp/clapper.m3u; clapper "${files[@]}"
